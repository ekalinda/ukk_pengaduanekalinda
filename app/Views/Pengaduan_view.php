            ...
            <div class="card-body">
                <table class="table table-striped table-bordered">
                        ...
                        foreach ($pengaduan as $row) {
                            $no++;
                            $data = $row['tgl_pengaduan'] . "," . $row['isi_laporan'] . "," . $row['foto']  . "," . base_url('pengaduan/' . $row['id_pengaduan']);
                        ?>
                            <tr>
                                <td><?= $no ?></td>
                                ...
                                <td>
                                  ...
                                  if(session('level')=='masyarakat'){
                                   if($row['status']=='0'){
                                  ?>
                                  <a onclick="return confirm('Yakin untuk hapus data?');"class="btn btn-danger" href="/pengaduan/hapus/<?=$row['id_pengaduan']?>"><i class="fa fa-trash"></i></a>
                                  <?php
                                   }else{
                                    //ini tempat melihat tanggapan
                                   }
                                  }
